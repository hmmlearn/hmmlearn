...
class TestKLDivergence:
    ...
    def test_multivariate_normal(self):
        mean_p = [0]
        var_p = [[1]]
        kl_equal = kl_divergence.kl_multivariate_normal_distribution(
            mean_p, var_p,
            mean_p, var_p)
        assert kl_equal == 0
        mean_q = [1]
        var_q = [[1]]
        kl_ne = kl_divergence.kl_multivariate_normal_distribution(
            mean_p, var_p,
            mean_q, var_q)
        assert kl_ne > 0
    ...
    def test_wishart(self):
        ...
        kl_equal = kl_divergence.kl_wishart_distribution(
            dof1, scale1,
            dof1, scale1)
        assert kl_equal == 0
        kl_ne = kl_divergence.kl_wishart_distribution(
            dof1, scale1,
            dof2, scale2)
        assert kl_ne > 0
